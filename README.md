...
## Fluxo para criar uma nova página

```
1. Identificar o template → Dashboard, CRUD, Auth, Galeria ou Wizard?
2. Mapear estados → loading, empty, error, populated
3. Verificar componentes → src/components/ui antes de criar
4. Usar tokens → nunca escrever cor ou radius literal
5. Estruturar JSX → hooks → queries → handlers → early returns → render
6. Testar responsividade → 320px, 768px, 1280px
7. Testar acessibilidade → tab order, aria-label, contraste
8. Validar dual theme → alternar dark/light e verificar
```

Ver detalhes: [design/docs/05-templates.md → Fluxo](./design/docs/05-templates.md#fluxo-para-criar-novas-páginas)
...
```
Visual
[ ] Tokens usados — nenhuma cor hardcoded
[ ] Hierarquia tipográfica respeitada (máx. 3 níveis por tela)
[ ] Border-radius e sombras dos tokens

Tema
[ ] Testado em dark (.dark no <html>)
[ ] Testado em light (sem .dark)
[ ] Glass bars com backdrop-filter em ambos

Responsividade
[ ] Mobile 320px — sem overflow horizontal
[ ] Desktop 1280px — espaço aproveitado
...
Este sistema funciona como **constituição visual**. Qualquer decisão de UI que contrarie as regras aqui definidas requer:
...
> Em caso de dúvida, consulte `design/design.md` antes de inventar um padrão novo.
...
| 2.0 | 2026-05-24 | Dual theme, CSS Variables, 19 seções, docs/, starter Next.js 16 |
